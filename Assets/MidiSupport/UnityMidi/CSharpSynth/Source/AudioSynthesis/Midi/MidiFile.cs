...
namespace AudioSynthesis.Midi
{
    ...
    {
        const int s_Microseconds_Per_Minute = 60000000;
        ...
        private double mBPM = 0; //1st BPM set of the track
        ...
        public void SetInitialBPM()
        {
            for (int x = 0; x < mTracks.Length; x++)
            {
                for (int y = 0; y < mTracks[x].MidiEvents.Length; y++)
                {
                    //Tempo set events
                   if(mTracks[x].MidiEvents[y].Data1 == 0x51)
                   {
                        MetaNumberEvent metaEvent = (MetaNumberEvent)mTracks[x].MidiEvents[y];
                        mBPM = s_Microseconds_Per_Minute/metaEvent.Value;
                        return;
                   }
                }  
            }
        ...
        {
            /*
            A0 in midi is the first note and its number is 21. 
            We adjust the index to start from 0 (hence noteNumber -= 21; at the beginning). 
            If your note numbers are 0 based, for example in piano from 0 to 88, then you can comment this line out.
            */
            noteNumber -= 21;
